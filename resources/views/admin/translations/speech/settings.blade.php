...
@section('content')
    <form action="{{ route('admin.translations.speech.settings.update', [$language, $content]) }}" method="post" autocomplete="off">
        @csrf
        ...
        @submit(['text' => 'Save'])
        @cancel(['route' => route('admin.translations.content.show', [$language, $content])])
    </form>
@endsection
