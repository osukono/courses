<form class="mb-3" action="{{ route('admin.dev.exercise.data.update', $data) }}" method="post" autocomplete="off"
    ...
    @input(['name' => 'extra_chunks', 'label' => '', 'default' => Arr::get($data->content, 'extra_chunks'), 'lg' => false, 'helper' => 'List of extra chunks (separated with comma) e.g. on, at, the'])
    @input(['name' => 'capitalized_words', 'label' => '', 'default' => Arr::get($data->content, 'capitalized_words'), 'lg' => false, 'helper' => 'List capitalized words (separated with comma)'])
